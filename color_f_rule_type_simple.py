...
WINDOW_NAME = "Bogie Colour + Height + OD + Type"
HISTORY_LENGTH = 15
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Detect spring, classify family by color, and classify type using OD and height.")
...
def classify_colour(mask: np.ndarray, image_bgr: np.ndarray) -> tuple[str, dict | None]:
    ...
    if (
        family == "CASNUB-22HS"
        and 90 <= mean_h <= 120
        and (sat_q75 >= 60 or high_sat_ratio >= 0.15)
    ):
        family = "CASNUB-22HS Mod-1 BOSTHS"
...
def process_frame(
    ...
    if best_box is not None:
        ...
        if hsv_stats is not None:
            ...
            cv2.putText(display,(f"HSV median: {hsv_stats['median_h']:.0f}, {hsv_stats['median_s']:.0f}, "f"{hsv_stats['median_v']:.0f} | pixels: {hsv_stats['pixel_count']} | "f"{hsv_stats['rule_name']} | type {spring_type}{od_text}"),(20, 40),cv2.FONT_HERSHEY_SIMPLEX,0.6,(0, 255, 255),2,cv2.LINE_AA,)
    else:
...
def run_single_image(
    ...
    display, overlay_text, _ = process_frame(
        frame, model, min_conf, float(calibration["pixels_per_mm"]), height_history_px, od_history_px
    )
